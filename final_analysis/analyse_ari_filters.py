...
base_path = "/home2/user/u0100486/linux/doctorate/github/tracker_sim/output/root_files_2/"
...
# --- Process one file ---
def process_file_for_filtered_aris(filepath):
    ...
    for event in tree:
        ...
        for method in ["ransac", "gmm"]:
            ...
            # Passed all filters — record ARIs
...
for energy in excitation_energies:
    for cm in cm_angles:
        for suppress in suppression_factor:
            ...
            for fpath in files:
                d = process_file_for_filtered_aris(fpath)
                for k in combined:
                    combined[k] += d.get(k, [])

            # --- Create canvas ---
            canvas = ROOT.TCanvas(f"c_{energy}_{cm}", f"Filtered ARI {energy} MeV, {cm}°", 1000, 700)
            ...
            # Draw first histogram, then overlay the rest
            ...
            out_png = f"noe1e2_filtered_ARI_all_{energy}MeV_{cm}cm_{suppress}.png"
