        ...
        for tree in [tree_1, tree_2]:
            for event in tree:
                ...
                if counter > 5200:
                    break
                counter += 1
                beta_ransac_tracks = list(event.beta_ransac_tracks)  # Track IDs (outer dictionary keys)
                beta_ransac_counts = list(event.beta_ransac_counts)  # Number of (range, angle) pairs per track
                beta_ransac = list(event.beta_ransac)  # Inner dictionary keys (ranges)
                beta_ransac_angle = list(event.beta_ransac_angle)  # Inner dictionary values (angles)

                phi_angles = np.array(event.ransac_phi_angles)
                ...
                ransac_end = np.array(event.ransac_end)
                ...
                if not np.any((8 <= ransac_end[:, 0]) & (ransac_end[:, 0] <= 248) &
                            (8 <= ransac_end[:, 1]) & (ransac_end[:, 1] <= 248) &
                            (8 <= ransac_end[:, 2]) & (ransac_end[:, 2] <= 248)):
